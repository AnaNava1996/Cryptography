...
int escritura(int c,int op){
  FILE *archi;
  if(op==1){
    archi=fopen("D:\\Users\\ana_n\\Documents\\IPN n.n\\6to Semestre\\Cryptography\\c.txt","a+");}
  if(op==2){
    archi=fopen("D:\\Users\\ana_n\\Documents\\IPN n.n\\6to Semestre\\Cryptography\\m.txt","a+");}//modificar para que borre
  fputc(c,archi);
  fclose(archi);}
...
int lectura(int op,int key,int alfa){
  ...
  if(op==2){
    arch=fopen("D:\\Users\\ana_n\\Documents\\IPN n.n\\6to Semestre\\Cryptography\\c.txt","r");
  }
  if(arch == NULL) {
